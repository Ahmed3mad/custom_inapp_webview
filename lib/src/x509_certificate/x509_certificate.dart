...
class X509Certificate {
  ...
  String get description => asn1?.fold("", (value, element) => (value ?? '') + element.description + '\n') ?? '';
  ...
  List<int> get serialNumber => block1?.atIndex(X509BlockPosition.serialNumber)?.value as List<int>;
  ...
  DateTime get notBefore => block1?.atIndex(X509BlockPosition.dateValidity)?.subAtIndex(0)?.value as DateTime;

  ///Gets the notAfter date from the validity period of the certificate.
  DateTime get notAfter {
    var value = block1?.atIndex(X509BlockPosition.dateValidity)?.subAtIndex(1)?.value as DateTime;
    return value;
  }
  ...
  List<String> get extendedKeyUsage => extensionObject(oid: OID.extKeyUsage)?.valueAsStrings ?? <String>[];

  ///Gets a collection of subject alternative names from the SubjectAltName extension, (OID = 2.5.29.17).
  List<String> get subjectAlternativeNames => extensionObject(oid: OID.subjectAltName)?.alternativeNameAsStrings ?? <String>[];

  ///Gets a collection of issuer alternative names from the IssuerAltName extension, (OID = 2.5.29.18).
  List<String> get issuerAlternativeNames => extensionObject(oid: OID.issuerAltName)?.alternativeNameAsStrings ?? <String>[];
  ...
  List<String> get criticalExtensionOIDs {
    ...
    return extensionBlocks.map((block) => X509Extension(block: block)).where((extension) => extension.isCritical).map((extension) => extension.oid ?? '').toList();
  }
  ...
  List<ASN1Object> get extensionBlocks => block1?.atIndex(X509BlockPosition.extensions)?.subAtIndex(0)?.sub;
  ...
  X509Extension extensionObject({String oidValue, OID oid}) {
    ...
    if (oidValue != null) {
      var block = block1?.atIndex(X509BlockPosition.extensions)?.findOid(oidValue: oidValue)?.parent;
      if (block != null) {
...
enum X509BlockPosition { version, serialNumber, signatureAlg, issuer, dateValidity, subject, publicKey, extensions }
